//tentacle example provider arb contract with flash loans

//Mainnet: 0xc6eba20877dc64e920975b9f5e6f1a0da48ee68e
...
/*
    ...
    function arbIt2(uint[] eOrder, string[] memory tOrder1, uint256 amount, bool back) {
        ...
        for(uint j=0; j<tOrder1.length; j++){
            tOrder[j] =tentacle.getTokenAddress(tOrder1[j]);
        }
      ...
      function arb(address fundsReturnToAddress, address liquidityProviderContractAddress, string[] memory tokens,  uint256 amount, string[] exchanges) payable returns(bool){
          if(liquidityProviderContractAddress == fundsReturnToAddress){
            ...
            latestFirstTokenAddress = tentacle.getTokenAddress(newTokens[0]);
            ...
            latestReturnAddress = fundsReturnToAddress;
             latestFirstTokenAddress = tentacle.getTokenAddress(tokens[0]);
...
*/
     ...
     arbIt3(eOrder1, tokens, amount, back);
         ...
         address lastToken = tentacle.getTokenAddress(tokens[tLast]);
