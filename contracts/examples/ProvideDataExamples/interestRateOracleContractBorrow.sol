...
function concat(string memory _base, string memory _value) public view  returns (string memory) {
    ...
    function getInterestRate(string memory symbol, PlatformSide memory platformSide) public view returns (uint256 ) {
        if (equal(platformSide.platform, "COMPOUND")) {
            ...
            Compound  cToken = Compound(tentacle.getTokenAddress(platformToken));
            ...
            Fulcrum  iToken = Fulcrum(tentacle.getTokenAddress(platformToken));
            ...
            if (platformSide.isBorrow) {
                return aave.getReserveCurrentVariableBorrowRate(tentacle.getTokenAddress(platformToken)).div(1e9);
            }
            return aave.getReserveCurrentLiquidityRate(tentacle.getTokenAddress(platformToken)).div(1e9);
        } else {
