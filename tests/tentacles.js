const Tentacle = artifacts.require('./tentacle.sol')
import assert, { equal } from 'assert'

contract('Tentacle', async accounts => {

    let NETWORK = process.env.NETWORK;

    before(async () => {
      if (NETWORK === 'mainnet') {
        contractInstance = await Tentacle.at("0x8316b082621cfedab95bf4a44a1d4b64a6ffc336")
      } else {
        contractInstance = await Tentacle.deployed()
      }
    })
